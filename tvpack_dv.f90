...
!
  ...
  ELSE
    ...
    IF (nu .LT. 1 .AND. abs3 + abs9 .LT. eps) THEN
      ...
      CALL BVTL_DV(nu, h2, h2d, h3, h3d, r23, r23d, result2, result2d, &
&            nbdirs)
      DO nd=1,nbdirs
    ...
    ELSE
      ...
      IF (nu .LT. 1 .AND. abs4 + abs10 .LT. eps) THEN
        ...
        CALL BVTL_DV(nu, h1, h1d, h2, h2d, r12, r12d, result2, result2d&
&              , nbdirs)
        DO nd=1,nbdirs
          tvtd(nd) = result1d(nd)*result2 + result1*result2d(nd)
        END DO
...
&                result2d, nbdirs)
        ...
        ELSE IF (1 - r23 .LT. eps) THEN
          ...
          ELSE
            ...
            min1 = h2
          END IF
          CALL BVTL_DV(nu, h1, h1d, min1, min1d, r12, r12d, tvt, tvtd, &
&                nbdirs)
...
&                  result1d, nbdirs)
          ...
          ELSE IF (r23 .GE. 0) THEN
            ...
            END DO
            CALL BVTL_DV(nu, h1, h1d, min2, min2d, zro, dummyzerodiffd, &
&                  tvt, tvtd, nbdirs)
...
&                  result1, result1d, nbdirs)
            CALL BVTL_DV(nu, h1, h1d, -h3, -h3d, zro, dummyzerodiffd1, &
...
!
SUBROUTINE BVTL_DV(nu, dh, dhd, dk, dkd, r, rd, bvtl, bvtld, nbdirs)
  USE DIFFSIZES
!  Hint: nbdirsmax should be the maximum number of differentiation directions
  IMPLICIT NONE
!     END BVTL
